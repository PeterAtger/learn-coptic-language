...
class GrammarEnPage extends StatefulWidget {
  const GrammarEnPage({super.key});

  @override
  State<GrammarEnPage> createState() => _GrammarEnPageState();
}

class _GrammarEnPageState extends State<GrammarEnPage> {
  ...
  Widget _buildTextWithMixedFonts(String text, {
    ...
    void addAutoDetectedSpans(String subtext) {
      ...
      bool isCopticChar(String char) {
        ...
        // In English mode, only assume Latin is Coptic if we're explicitly in a legacy block
        return isLegacy && RegExp(r"[a-zA-Z`\\\[\]:;'\/?\><|~{}]").hasMatch(char);
      }
  ...
  Future<void> _loadData() async {
    ...
    try {
      // 1. Try to load stage-specific file: grammar_stageId_en.json
      final String jsonPath = 'assets/data/grammar_${stageId}_en.json';
